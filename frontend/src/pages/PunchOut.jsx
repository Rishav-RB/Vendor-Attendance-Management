...
const VendorSignup = () => {
    ...
    useEffect(()=>{
        const isValid=async()=>{
            try {
                ...
                const res=await fetch(`/test/attendance/getValidity/${loggeduser.user_id}`,{
                    method:'GET',
                    headers:{
  ...
  return (
    ...
    <div className='pt-0 mt-0 bg-gradient-to-b from-red-400 to-red-900 text-white w-full h-[36px] rounded-md flex justify-between items-center'>
        ...
        <p className='absolute left-1/2 transform -translate-x-1/2'>Vendor Punch-Out</p>
    ...
    <div className='bg-gray-100 min-h-screen flex justify-start items-center flex-col text-black'>
        ...
        <div className="bg-gray-white pb-6 border-black border-[0.1px] w-[1520px] rounded-md">
            ...
            <div className="form-control">
                    ...
                    value={inputs.sp_no} onKeyDown={handleChange}
                    onChange={(e)=>setInputs({...inputs,sp_no:e.target.value})} />
                </div>
        </form>
        <div className="form-control my-6 ">
            <button className={`w-[1520px] btn hover:bg-gray-400 hover:text-black text-white bg-gray-800`} disabled={!valid} onClick={()=>logout()}>Log Out</button>
        </div>
        <div className='flex flex-row justify-start items-start m-1 text-black mr-auto'>
